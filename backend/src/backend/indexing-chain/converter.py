...
def convert_to_md(html_document: HTMLDocument) -> MarkdownDocument:
    return MarkdownDocument(
        source_url=html_document.source_url,
        markdown=md(
            html_document.html,
...
def convert_to_mds(html_documents: List[HTMLDocument]) -> List[MarkdownDocument]:
    return [convert_to_md(html_document) for html_document in html_documents]
